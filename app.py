...
if menu == "서류 분석 / 생성 Master":
    ...
    if task['status'] == 'completed' and task['type'] == 'doc_parse':
        ...
        # 타사 인풋 서류 헤더 정보를 고객사 대장 저장용으로 자동 분리 및 저장
                ...
                if st.button("🚀 스마트 일괄 자동 등록 ", key="btn_reg_all_batch", disabled=is_running):
                    if reg_pwd != SAVE_PASSWORD: st.error(t("pwd_err"))
                        ...
                        if 'parsed_input_doc' in st.session_state:
                            ...
                            save_to_doc_ledger(
                                CUSTOMER_DB_FILE, 
                                doc_type=inp.get("DocType", "Invoice"), 
                                your_ref=inp.get("YourRef", "-"), 
                                our_ref="-", 
                                ship_name=inp.get("ShipName", "-"), 
                                target_name=inp.get("TargetName", "-"), 
                                doc_date_str=inp.get("DocDate", "-"), 
                                currency=inp.get("Currency", "KRW"), 
                                total_amount=inp.get("TotalAmount", 0.0), 
                                item_count=inp.get("ItemCount", 1), 
                                user_email=inp.get("CreatedBy", "External")
                            )

                        # 2. 새로 생성된 자사 아웃풋 서류는 '자사 서류 대장'에 저장
                        save_to_doc_ledger(
                            OUR_DB_FILE, 
                            doc_type=doc_type, 
                            your_ref=your_ref, 
